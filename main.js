...
function validateEmail() {
    ...
    if(emailInput.value===""){
      emailError.textContent="First Name should not be empty."
    }
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === XMLHttpRequest.DONE) {
      if (xhr.status === 200) {
        ...
        var phoneCodes = response.map(country => country.callingCodes[0]);
        populateSelectBox(phoneCodes);
      } else {
...
function populateSelectBox(phoneCodes) {
  var selectElement = document.getElementById('phoneCodeSelect');
...
function signUp() {
    var user = {};
     ...
     user.emailInput = document.getElementById("email").value;
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState === 4) {
          resultElement.innerHTML = "Sign up successful!";
      } else {
...
document.getElementById("countryNames").addEventListener("change", countryOnchange);
...
function countryOnchange(){
  ...
  unihttp.onload = function() {
    let universityNames = JSON.parse(this.responseText);
...
function validatePhoneNumber() {
  ...
  if (!phoneRegex.test(phoneInput)) {
      const errorMessage = "Please enter a 10-digit phone number without spaces, dashes, or special characters.";
      document.getElementById('error1-message').textContent = errorMessage;
  } else {
      document.getElementById('error1-message').textContent = "";
  }
}
